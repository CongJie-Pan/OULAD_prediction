...
a. 維持現狀並調整 Threshold（決策閾值）： 模型架構不用動了。在預測時，將判斷標準從 0.5 改成 0.45 或 0.4，這樣可以進一步把 Recall 推高到 80% 以上（代價是會有一些 safe 的學生被誤判為 risky，需要輔導老師多費點心確認）。

b. 折衷方案： 現在試過了 Dropout 0.2 (稍微 Underfitting / 差距大) 和 Dropout 0.1 (稍微 Overfitting / 差距小)。也許 Dropout = 0.15 會是這個資料集的「黃金甜蜜點」。
