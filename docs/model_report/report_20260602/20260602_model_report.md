...
**3. 下一步行動計畫：**
*   **納入測驗成績與自動調參**：
    *   **導入平時測驗成績 (Assessment Data)**：若需納入每次的評分成績作為動態特徵，這等同於要重寫 3D-Array 資料清洗模組，以對齊時間序列。本專案目前專注於學習行為預測，未來可將成績資料設計為 Multi-Input 的第三分支。
    *   **超參數最佳化 (Hyperparameter Tuning)**：未來可導入 `KerasTuner` 進行自動調參，但因 Notebook 執行時間將大幅暴增至三到四個小時以上，目前版本以實證過的架構為準。
    *   *(註：強烈建議這部份作為期末簡報結尾的「未來展望」頁面進行口頭報告即可，既能展現團隊對後續擴充的規劃與理解，又能避免在現有成熟專案上引發過大的重構風險。)*